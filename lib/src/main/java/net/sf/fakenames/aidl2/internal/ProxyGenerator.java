...
final class ProxyGenerator extends AptHelper implements AidlGenerator {
        ...
        if (!model.methods.isEmpty()) {
            ...
            for (AidlMethodModel method : model.methods) {
                ...
                final CodeBlock flags = method.oneWay
                        ? net.sf.fakenames.aidl2.internal.util.Util.literal("$T.FLAG_ONEWAY", iBinder)
                        : net.sf.fakenames.aidl2.internal.util.Util.literal("0");
                ...
                for (AidlParamModel param : method.parameters) {
                    ...
                    if (param.inParameter) {
                        ...
                        } catch (net.sf.fakenames.aidl2.internal.exceptions.CodegenException cge) {
                            throw new ElementException(cge, net.sf.fakenames.aidl2.internal.util.Util.arg(method.element.element, param.name));
                        }
                    }
                }
    ...
    private void writeInParam(CodeBlock.Builder code, State writer, String parcel) throws net.sf.fakenames.aidl2.internal.exceptions.CodegenException {
        writer.buildWriter(parcel).write(code, writer.type);

        code.add("\n");
    }

    private void readReturnValue(CodeBlock.Builder code, State reader, String name) throws net.sf.fakenames.aidl2.internal.exceptions.CodegenException {
