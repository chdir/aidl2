...
public final class Writer extends AptHelper {
    ...
    public Writer(AidlProcessor.Environment environment, State state, CharSequence outParcelName) {
        ...
        if (state.returnValue) {
            flags = net.sf.fakenames.aidl2.internal.util.Util.literal("$T.PARCELABLE_WRITE_RETURN_VALUE", parcelable);
        } else {
            flags = 0;
        }
    }

    public TypeMirror write(CodeBlock.Builder paramWriting, TypeMirror type) throws net.sf.fakenames.aidl2.internal.exceptions.CodegenException {
    ...
    private Strategy getStrategy(TypeMirror type) throws net.sf.fakenames.aidl2.internal.exceptions.CodegenException {
        switch (type.getKind()) {
            case BOOLEAN:
            case INT:
            case SHORT:
            ...
            case DECLARED:
                final CharSequence name = net.sf.fakenames.aidl2.internal.util.Util.getQualifiedName((DeclaredType) type);
                if (name != null) {
    ...
    private Strategy getExternalizableStrategy(TypeMirror type) {
        return Strategy.create((block, name) -> {
            ...
            block.addStatement("$T.shut($N)", net.sf.fakenames.aidl2.AidlUtil.class, oos);
            block.endControlFlow();
        }, externalizable);
    }

    private Strategy getParcelableStrategy() {
        return Strategy.create((block, name) -> block.addStatement("$L.writeToParcel($L, $L)", name, parcelName, flags), parcelable);
    }

    private Strategy getBuiltinStrategy(TypeMirror type) throws net.sf.fakenames.aidl2.internal.exceptions.CodegenException {
        ...
        switch (componentKind) {
            ...
            default:
                if (types.isSubtype(component, parcelable)) {
                    return getSpecialArrayStrategy(getParcelableStrategy(), component);
                }
    ...
    private Strategy getSpecialArrayStrategy(Strategy delegate, TypeMirror component) {
        ...
        return Strategy.createNullSafe((block, name) -> {
            ...
            final boolean nullable = net.sf.fakenames.aidl2.internal.util.Util.isNullable(component, this.nullable);

            // now with casts!
            final CodeBlock elementBlock = emitFullCast(resultType, requestedType, net.sf.fakenames.aidl2.internal.util.Util.literal(element));
    ...
    private static class Strategy implements WritingStrategy {
        ...
        @Override
        public void write(CodeBlock.Builder block, Object name) throws net.sf.fakenames.aidl2.internal.exceptions.CodegenException {
            delegate.write(block, name);
        }
    }

    private interface WritingStrategy {
        void write(CodeBlock.Builder block, Object name) throws net.sf.fakenames.aidl2.internal.exceptions.CodegenException;
    }
}
